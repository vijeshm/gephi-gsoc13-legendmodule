...
public class AttributeRowImpl implements AttributeRow {
    ...
    protected final Attributable source;
    protected final AttributeTableImpl attributeTable;
    protected AttributeValueImpl[] values;
    protected int rowVersion = -1;

    public AttributeRowImpl(AttributeTableImpl attributeTable, Attributable source) {
    ...
    public void reset() {
        ...
        for (int i = 0; i < attSize; i++) {
            AttributeColumnImpl tableCol = attributeTable.getColumn(i);
            newValues[i] = (AttributeValueImpl) attributeTable.getFactory().newValue(tableCol, this, tableCol.getDefaultValue());
            tableCol.putValue(newValues[i]);
        }
    ...
    public void setValues(AttributeRow attributeRow) {
        ...
        for (int i = 0; i < attValues.length; i++) {
            setValue(attValues[i].getColumn(), attValues[i].getValue());
        }
    }
    ...
    public void setValue(AttributeColumn column, Object value) {
        ...
        AttributeValue attValue = attributeTable.getFactory().newValue(column, this, value);
        setValue(attValue);
    }
    ...
    public void setValue(AttributeValue value) {
        ...
        if (attributeTable.getColumn(column.getIndex()) != column) {
            ...
            value = attributeTable.getFactory().newValue(column, this, value.getValue());
        }
    ...
    private void setValue(int index, AttributeValueImpl value) {
        ...
        oldValue.getColumn().removeValue(oldValue);
        value.getColumn().putValue(value);
        ...
        if (!((oldValue == null && value == null) || (oldValue != null && oldValue.equals(value)))
            ...
            attributeTable.model.fireAttributeEvent(new ValueEvent(EventType.SET_VALUE, attributeTable, source, value));
        }
    }
    ...
    public void removeFromDictionary() {
        if (values != null) {
            for (AttributeValue value : values) {
                ((AttributeColumnImpl) value.getColumn()).removeValue(value);
            }
        }
    }
